...
\bigskip
\input{tables/algorithm_comparison}
...
\paragraph{Expected input shape.}
A flattened one-dimensional feature vector of length $d$.  When
using magnitude-only encoding, $d = 4001$; with magnitude-plus-phase
dual-channel encoding, $d = 8002$.
...
\paragraph{Expected input shape.}
Flattened 1D vector of length $d$ for Route~A inputs.  Route~B inputs
(2D images) can also be flattened, although this discards spatial
structure.  Thus, the MLP is compatible with both routes in principle
but benefits from neither's structure.
...
\paragraph{Expected input shape.}
A 1D sequence of length $L = 4001$ with $C$ channels, where $C = 1$
for magnitude-only or $C = 2$ for magnitude-plus-phase encoding.

\paragraph{Compatible route.}
Route~A (primary).  Multi-channel 1D input naturally accommodates
the dual-channel representations discussed in \Cref{sec:repr}.
...
\paragraph{Expected input shape.}
1D sequence, identical to the 1D CNN.
...
\paragraph{Expected input shape.}
Flattened 1D feature vector, Route~A.
