...
The lowest SNR levels ($10$--$15$~dB) represent \emph{extreme stress
conditions} that exceed the noise levels expected in typical VNA
measurements; they are included to identify the model's breaking point,
not to simulate realistic operating scenarios.  In practice, a
well-calibrated VNA in the \SIrange{8}{12}{\giga\hertz} band achieves
$\text{SNR} \geq 40$~dB under normal laboratory conditions.  The value
at which each model's macro~F1 drops below a threshold (e.g., $0.90$)
provides a quantitative ``robustness margin'' for cross-model
comparison.
...
%
\begin{enumerate}
  \item Three-channel hybrid (dB, Re, Im; $L = 4001 \times C = 3$)
        \textbf{[primary]}.
  \item Magnitude only ($|S_{11}|$ in dB, $d = 4001$).
  \item Cartesian only (Re, Im; $L = 4001 \times C = 2$).
