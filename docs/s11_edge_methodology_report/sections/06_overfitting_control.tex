...
\paragraph{Recommended implementation: \texttt{StratifiedGroupKFold}.}
When near-duplicate or batch-correlated traces are identified
(\Cref{sec:overfit:duplicates}), both class stratification and group
awareness must hold \emph{simultaneously}.  Using
\texttt{StratifiedKFold} alone risks splitting near-duplicate groups
across folds; using \texttt{GroupKFold} alone may produce class-
imbalanced folds.  The unified solution is
\texttt{StratifiedGroupKFold} (scikit-learn $\geq$\,0.24), which
enforces both constraints in a single splitter object.  When no
near-duplicate groups are detected, standard \texttt{StratifiedKFold}
is sufficient.
