...
The dataset is acquired in native polar format ($|S_{11}|$ in~dB,
$\angle S_{11}$ in degrees).  To preserve the high dynamic range of the
magnitude---critical for the deep \SI{-62}{\decibel} unloaded
resonance---while simultaneously avoiding phase-wrapping phenomena, the
pipeline constructs a \textbf{3-channel tensor representation:}
$[\,|S_{11}|\text{ (dB)},\;\text{Re}[S_{11}],\;\text{Im}[S_{11}]\,]$.
The dB channel retains the full \SI{45}{\decibel} dynamic range as a
high-contrast feature, while the Cartesian channels provide
mathematically continuous phase information free of
$\pm\SI{180}{\degree}$ discontinuities.
...
\begin{description}
    \item[Route~A --- 1D Spectral:]  The three-channel frequency-domain vectors
        are retained as one-dimensional feature sequences.  The primary encoding
        is a tensor $\mathbf{x} \in \mathbb{R}^{4001 \times 3}$ containing the
        dB magnitude, Real, and Imaginary components.  Baseline ablations may
        evaluate $C=1$ (dB only) or $C=2$ (Re, Im only).  This route preserves
        full numerical fidelity and is compatible with both classical tabular
        models (via flattening) and 1D neural architectures.
