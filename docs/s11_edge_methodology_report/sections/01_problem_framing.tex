...
this methodology strictly adopts the \textbf{Cartesian representation} for machine
learning:
\begin{equation}
    S_{11}(f) \;=\; \text{Re}[S_{11}(f)] + j \cdot \text{Im}[S_{11}(f)].
    \label{eq:s11_cartesian}
\end{equation}

The Cartesian form is mathematically superior for training gradient-based learning
algorithms because it entirely eliminates the ``phase wrapping'' problem—the
artificial discontinuity that occurs when phase angles wrap from $\SI{+180}{\degree}$
to $\SI{-180}{\degree}$. Both the real and imaginary components are continuous,
well-behaved functions of frequency, natively providing a uniform numerical scale
for optimisation without requiring heuristic unwrapping algorithms.
