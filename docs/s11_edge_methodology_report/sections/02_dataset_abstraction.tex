...
lines and no header row.  Empirical inspection of the raw values
confirms that the data is stored in \textbf{DB format}
(magnitude in decibels, phase in degrees), not Real-Imaginary.
The absence of the option line means that:
...
\noindent
A robust ingestion pipeline should therefore prepend the correct
synthetic option line: \texttt{\# HZ S DB R 50}.  The empirical data
confirms the VNA exported the S-parameters in dB/Phase format, not
Real/Imaginary.  Injecting an \texttt{RI} format code would corrupt
the magnitude interpretation, causing parsers to treat decibel values
as linear real components.
...
\begin{tabular}{cll}
    ...
    2 & $|S_{11}|$ (Magnitude) & \si{\decibel} \\
    3 & $\angle S_{11}$ (Phase) & degrees \\
    \bottomrule
\end{tabular}
\end{center}
...
Magnitude values are negative (e.g.\ \SI{-23.541}{\decibel}),
consistent with a passive reflective structure where $|S_{11}| < 1$
in linear scale.  Phase values span the full
$\pm\SI{180}{\degree}$ range.

% ─────────────────────────────────────────────────────────────────────
\subsection{Sample Trace Inspection}
\label{sec:dataset:sample}

\Cref{tab:sample_trace} reproduces the first five rows of
...
\num{1603} samples.  The table shows both the native dB/Phase values as
stored in the file and the derived three-channel representation used by
the learning pipeline (see \Cref{sec:preproc} for the conversion formulae).

\begin{table}[htbp]
    \centering
    \caption{%
        First five rows of \texttt{Trc0.s1p} (\texttt{dry} class).
        Columns~2--3 are the native dB/Phase values read directly from
        the file; columns~4--6 show the three-channel tensor constructed
        by the preprocessing pipeline.%
    }
    \label{tab:sample_trace}
    \small
    \begin{tabular}{r r rr rrr}
        \toprule
        & & \multicolumn{2}{c}{\textbf{Native format}} &
          \multicolumn{3}{c}{\textbf{3-channel representation}} \\
        \cmidrule(lr){3-4} \cmidrule(lr){5-7}
        \textbf{Row} &
        \textbf{Freq.\ (\si{\giga\hertz})} &
        \textbf{$|S_{11}|$ (\si{\decibel})} &
        \textbf{$\angle S_{11}$ ($^{\circ}$)} &
        \textbf{Ch\,1: dB} &
        \textbf{Ch\,2: Re} &
        \textbf{Ch\,3: Im} \\
        \midrule
        1 & 8.000 & $-23.541$ & $-167.978$ & $-23.541$ & $-0.065$ & $-0.014$ \\
        2 & 8.001 & $-23.457$ & $-168.916$ & $-23.457$ & $-0.066$ & $-0.013$ \\
        3 & 8.002 & $-23.721$ & $-170.477$ & $-23.721$ & $-0.064$ & $-0.011$ \\
        4 & 8.003 & $-23.531$ & $-174.411$ & $-23.531$ & $-0.066$ & $-0.006$ \\
        5 & 8.004 & $-23.811$ & $-174.414$ & $-23.811$ & $-0.064$ & $-0.006$ \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item The step size is not an exact integer number of hertz,
        ...
        intervals.
    \item Magnitude values are approximately \SI{-23.5}{\decibel},
        consistent with a heavily reflective (poorly matched) passive
        structure at these frequencies.
    \item Phase values near \SI{-168}{\degree} at \SI{8}{\giga\hertz}
        cross through $\pm\SI{180}{\degree}$ within the first few points
        (note the sign change between rows~7 and~8 in the full file).
        This phase-wrapping discontinuity in the native format is
        precisely what motivates the conversion to Cartesian
        ($\text{Re}$, $\text{Im}$) channels, which are mathematically
        continuous.
    \item The derived Re and Im values (channels~2 and~3) are small
        negative numbers near zero, consistent with $|S_{11}| \ll 1$ in
        linear scale ($10^{-23.5/20} \approx 0.067$).
\end{itemize}
...
biased evaluation.  Synthetic minority oversampling (e.g.\ SMOTE) is
\textbf{explicitly prohibited} for this dataset; see
\Cref{sec:augment:smote_prohibition} for the physical justification.
